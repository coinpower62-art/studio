...
export function DashboardClient({ initialData }: { initialData: any }) {
  ...
  const processUsers = (data: any) => {
    ...
    const allGenerators = data.generators || [];
    const rentedGensRaw = data.rentedGenerators || [];
    const generatorMap = new Map(allGenerators.map((g: Generator) => [g.id, g.name]));

    const rentedByUser = rentedGensRaw.reduce((acc: any, rg: RentedGeneratorRaw) => {
    ...
    const referralCounts = rawUsers.reduce((acc: any, user: UserRecord) => {
      if (user.parent_id) {
    ...
    return rawUsers.map((user: UserRecord) => ({
      ...user,
      referral_count: referralCounts[user.id] || 0,
      rented_generators: (rentedByUser[user.id] || []).sort((a:any, b:any) => new Date(b.expires_at).getTime() - new Date(a.expires_at).getTime()),
    }));
  }
  ...
  const [users, setUsers] = useState<UserRecord[]>(processUsers(initialData));
  const [generators, setGenerators] = useState<Generator[]>(initialData.generators || []);
  const [deposits, setDeposits] = useState<DepositRequest[]>(initialData.deposits || []);
  const [withdrawals, setWithdrawals] = useState<WithdrawalRecord[]>(initialData.withdrawals || []);
  const [media, setMedia] = useState<MediaAsset[]>(initialData.media || []);
  const [bonusCodes, setBonusCodes] = useState<GiftCode[]>(initialData.codes || []);
  const [visits, setVisits] = useState<{date: string, view_count: number}[]>(initialData.visits || []);
  const [allRentedGenerators, setAllRentedGenerators] = useState<RentedGeneratorRaw[]>(initialData.rentedGenerators || []);
  ...
  const fetchData = async () => {
      ...
      if (result.error || !result.data) {
        ...
        setUsers(processUsers(result.data));
        ...
        setAllRentedGenerators(result.data.rentedGenerators as RentedGeneratorRaw[] || []);
      }
  ...
  const handleCreateGenerator = async () => {
    ...
    if(result.error || !result.data) {
      ...
      setShowCreateGen(true); // wait, it was set to true? Corrected below
